...
var transporter = nodemailer.createTransport({
  ...
  secure: false,
  auth: {
    user: 'forotpverification@outlook.com',
    ...
    UserDetails.findOne({ email: Newuser.email }, (err, user) => {
      if (user) {
        ...
        try {
          ...
          var mailOptions = {
            from: 'forotpverification@outlook.com',
            to: Newuser.email,
          ...
          transporter.sendMail(mailOptions, async function (error, info) {
            if (error) {
              console.log(info);
              res.send({ message: "error in sending mail" });
            }
            else {
...
app.post("/verify-otp", async (req, res) => {
  ...
  if (code == usercode) {
    const hash=await bcrypt.hashSync(Rbody.password, saltRounds);
    await Saveuserdata({ email: Rbody.email, password: hash });
...
app.post("/loginPage", (req, res) => {
  ...
  if (Newuser.email === null) res.send({ message: "Please enter Email" });
    ...
    UserDetails.findOne({ email: Newuser.email }, (err, user) => {
      if (user) {
        var same=bcrypt.compareSync(Newuser.password, user.password)
        if (same) {
...
app.put("/update-password", auth, async (req, res) => {
  ...
  try {
    UserDetails.findOne({ email: mail }, async (err, user) => {
      if (err) res.send({ message: err });
      else {
